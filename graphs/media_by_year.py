...
def run():
    sql = "SELECT album.yearreleased as `Year of Release`, source.Source, count(albumid) as Albums FROM album inner join Source on album.SourceID = Source.SourceID where album.albumtypeid<>16 group by album.yearreleased, source.source order by album.yearreleased, Source.Source;"
